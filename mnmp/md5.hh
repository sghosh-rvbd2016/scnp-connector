...
#define MD5_BUFFER_LENGTH 64

class MazuMD5Digest {
  public: 
    void set(const unsigned char *digest) {
      memcpy(&_digest, digest, MD5_DIGEST_LENGTH);
    }
    const unsigned char *get() const { return &_digest[0]; }
  private:
...
public:
  ...
  bool get_digest(MazuMD5Digest &in) const;

  static std::string digest_string(std::string &s);
...
private:
  ...
  uint1 buffer[MD5_BUFFER_LENGTH];   // input buffer
  uint1 digest[MD5_DIGEST_LENGTH];
  uint1 finalized;
